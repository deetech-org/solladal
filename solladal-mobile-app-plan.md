...
Generate these from source images with `npx capacitor-assets generate` (the `@capacitor/assets` dev dependency from §6 Step 1):
...
2. In the top toolbar device selector, pick **`Pixel\_8`**.

3. Click the green **Run (▶)** button (or press `Shift + F10`).

4. Android Studio compiles, boots the emulator, and attaches live Webview & Logcat debugging.
...
- [x] **Fonts self-hosted** and runtime Google Fonts `\<link\>`s removed; CSP added (Blocker 1).

- [x] **No "sensitive information"** in app name, subtitle, keywords, or screenshots (Blocker 2).

- [x] **Stats persistence** migrated to native storage via `@capacitor/preferences` (Blocker 3).

- [x] `python scripts/sync\_wordbank.py` run clean; `data/words.json` current and bundled.

### Apple App Store Connect Checklist

- [x] **Account:** Apple Developer Program (`deetech.org`).

- [x] **App ID:** `org.deetech.solladal` with no special entitlements needed.
...
- [x] **Account:** Google Play Developer Account (`deetech.org`).

- [x] **Package:** `org.deetech.solladal`.

- [x] **Target API Level:** meet Google's current minimum for new apps (Android 15 / API 35 configured).
...
2. **Android Asset & Icon Generation:**
   ...
   - Prepare icon assets in `assets/mobile/` and run asset generator if needed.
